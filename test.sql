...
CREATE DEFINER=`root`@`%` PROCEDURE `transfer_cycle_partition`(p_cycle_id VARCHAR(10))
BEGIN
  ...
  DECLARE staging_areas_cursor CURSOR FOR SELECT 	source_table, target_table_id, fact_table_name, IFNULL(on_duplicate_clause,''),	staging_partition_field, IFNULL(post_transfer_sp,''), aggr_date_field, hour_id_field, post_transfer_aggregations, IFNULL(reset_aggregations_min_date, '0000-00-00'), ignore_duplicates_on_transfer, reaggr_percent_trigger
